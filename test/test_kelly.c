...
static void test_transaction_costs(void)
{
    ...
    /* First compute what unconstrained Kelly would be */
    double f_kelly[2];
    kelly_multi_asset(mu, S, n, INFINITY, 0.5, f_kelly, work);

    /* Very high transaction cost: should stay at f_prev
     * Need lambda > |f_kelly - f_prev| to prevent any movement */
    double max_delta = fmax(fabs(f_kelly[0] - f_prev[0]), fabs(f_kelly[1] - f_prev[1]));
    double lambda_high = max_delta + 0.1; /* Ensure no movement */

    kelly_multi_asset_with_costs(mu, S, f_prev, n, INFINITY, 0.5, lambda_high, f_out, work);

    /* Changes should be zero */
    TEST_ASSERT_NEAR(f_out[0], f_prev[0], 1e-6, "High tcost keeps position 0");
    TEST_ASSERT_NEAR(f_out[1], f_prev[1], 1e-6, "High tcost keeps position 1");
...
static void test_ukf_integration(void)
{
    ...
    F[0] = 1.0;
    F[4] = 1.0;
    F[8] = 0.95; /* Diagonal, log_vol mean-reverts */
...
static void test_full_pipeline(void)
{
    ...
    /* Get NIS stats */
    srukf_enable_nis_tracking(ukf, 20, 3.0); /* Enable tracking first */

    /* Run more updates to populate NIS window */
    for (int i = 0; i < 20; i++)
    {
        z[0] = 100.0 + 0.05 * i + 0.3 * ((double)rand() / RAND_MAX - 0.5);
        srukf_step(ukf, z);
    ...
    SRUKF_NIS_Stats nis_stats;
    srukf_get_nis_stats(ukf, &nis_stats);
    double nis_above = (nis_stats.window_fill > 0)
                           ? (double)nis_stats.n_outliers / nis_stats.window_fill
                           : 0.0;
    ...
    srukf_get_nis_stats(ukf, &nis_stats);
    nis_above = (nis_stats.window_fill > 0)
                    ? (double)nis_stats.n_outliers / nis_stats.window_fill
                    : 0.0;
