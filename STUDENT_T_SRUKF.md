...
│       ▼                                                     │
│   ┌───────┐                                                 │
│   │  SSA  │  Singular Spectrum Analysis (0.6ms)             │
│   └───┬───┘  - Extracts trend, structure                    │
│       │      - Data-adaptive, no imposed model              │
│       ▼                                                     │
│   ┌───────┐                                                 │
│   │ BOCPD │  Bayesian Online Changepoint Detection (ns)     │
│   └───┬───┘  - Detects regime shifts                        │
│       │      - Triggers model updates                       │
│       ▼                                                     │
│   ┌───────────────────┐                                     │
│   │ Student-t SQR UKF │  ◄── THIS IMPLEMENTATION (2.4µs)    │
│   └─────────┬─────────┘  - Tracks state with fat-tail       │
│             │               robustness                      │
│             │            - State-dependent volatility       │
│             ▼                                               │
│   ┌───────┐                                                 │
│   │ Kelly │  Position Sizing                                │
│   └───┬───┘  - Uses state estimate + uncertainty            │
│       │                                                     │
│       ▼                                                     │
│   ┌─────────────┐                                           │
│   │ Kill Switch │  Risk Management                          │
│   └─────────────┘  - Monitors NIS, volatility, divergence   │
│                    - Halts trading if model degrades        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
SSA:      "What structure exists now?"      (data-driven)
BOCPD:    "Did it just change?"             (detection)
UKF:      "Smooth the noise"                (simple model)
Kelly:    "Size for uncertainty"            (optimal sizing)
```
