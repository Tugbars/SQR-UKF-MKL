...
#define KELLY_TAIL_FLOOR_NU 2.1 /* Below this ν, infinite variance */
    ...
    {
        /* Gaussian case: no tail adjustment */
        if (!isfinite(nu))
        {
            return base_var;
        }
    ...
    static inline double kelly_simple(double mu, double sigma, double nu, double fraction)
    {
        ...
        /* Clamp */
        if (f > KELLY_MAX_LEVERAGE)
            f = KELLY_MAX_LEVERAGE;
        if (f < KELLY_MIN_LEVERAGE)
            f = KELLY_MIN_LEVERAGE;
        ...
        double *work)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (f_out[i] > KELLY_MAX_LEVERAGE)
                f_out[i] = KELLY_MAX_LEVERAGE;
            if (f_out[i] < KELLY_MIN_LEVERAGE)
                f_out[i] = KELLY_MIN_LEVERAGE;
        }
